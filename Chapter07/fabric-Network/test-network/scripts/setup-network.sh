...
# Create the channel
docker exec -e "CORE_PEER_MSPCONFIGPATH=/etc/hyperledger/msp" peer0.org1.example.com peer channel create -o orderer.example.com:7050 -c mychannel -f /etc/hyperledger/configtx/channel.tx

# Join the peer to the channel
docker exec -e "CORE_PEER_MSPCONFIGPATH=/etc/hyperledger/msp" peer0.org1.example.com peer channel join -b mychannel.block
